...
### Неидеальность Proxy (магия Vue Proxy)

Представим, что у нас в оборачиваемом Proxy значении (в том же `ref`) лежит не просто число, как в примерах выше, а объект:
...
// потому что тут срабатывает магия Vue: 
// внутренние объекты значений он тоже оборачивает в Proxy.
setInterval(() => {
...
С counterState.value.counter реактивность продолжает работать, потому что тут срабатывает магия `Vue`: внутренние объекты значений он тоже оборачивает в `Proxy`.

Поэтому есть общий совет:

мы стремимся как можно раньше обернуть сущность в `ref` и продолжать работу уже с её `.value`.